...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-8">
        ...
        <div class="mb-4 sm:mb-0">
            <h1 class="text-2xl md:text-3xl text-gray-800 font-bold">Thêm kế hoạch mới</h1>
        </div>
        ...
        <form action="{{ route('admin.plans.store') }}" method="POST">
            ...
            <div class="space-y-6">
                ...
                <div class="border-b border-gray-200 pb-6">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="status" id="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            <option value="pending" {{ old('status') == 'pending' ? 'selected' : '' }}>Chờ thực hiện</option>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="plan_note" id="plan_note" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('plan_note') }}</textarea>
                        @error('plan_note')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex justify-end">
                    <button type="submit" class=" flex items-center  p-[5px]  rounded-sm    btn bg-indigo-500 hover:bg-indigo-600 text-white">Tạo kế hoạch</button>
                </div>
            </div>
        </form>
    </div>
