...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-8">
        ...
        <div class="mb-4 sm:mb-0">
            <h1 class="text-2xl md:text-3xl text-gray-800 font-bold">Thêm thành viên câu lạc bộ mới</h1>
        </div>

        <!-- Right: Actions -->
        <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            <a href="{{ route('admin.member.index') }}" class="btn border-gray-200 hover:border-gray-300 text-gray-600">
            ...
            <div class="space-y-4">
                ...
                <div class="border-b border-gray-200 pb-4 mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="student_code" id="student_code" value="{{ old('student_code') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        @error('student_code')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="enrollment_year" id="enrollment_year" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            ...
                            <option value="{{ $year }}" {{ old('enrollment_year') == $year ? 'selected' : '' }}>{{ $year }}</option>
                            @endfor
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="major" id="major" value="{{ old('major') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        @error('major')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex justify-end pt-5">
                    <button type="submit" class="btn bg-indigo-500 hover:bg-indigo-600 text-white">Tạo thành viên</button>
                </div>
            </div>
        </form>
    </div>
