...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-8">
        ...
        <div class="mb-4 sm:mb-0">
            <h1 class="text-2xl md:text-3xl text-gray-800 font-bold">Danh sách thành viên tham gia</h1>
        </div>
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="text-right flex justify-end items-center space-x-2">
                                ...
                                <form action="{{ route('admin.account.toggle-status', $member) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="admin-action-btn {{ $member->status == 'inactive' ? 'admin-btn-warning' : 'admin-btn-success' }}" title="{{ $member->status == 'inactive' ? 'Vô hiệu hóa' : 'Kích hoạt' }}">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            @if($member->status == 'inactive')
