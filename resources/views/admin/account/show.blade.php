...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-8">
        ...
        <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            ...
            <a href="{{ route('admin.account.edit', $user) }}" class=" p-[5px]  rounded-sm flex items-center btn bg-indigo-500 hover:bg-indigo-600 text-white">
            ...
            <div class="mt-8 border-t pt-6 flex items-center justify-between">
                <div>
                    <form action="{{ route('admin.account.toggle-status', $user) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class=" flex items-center btn border-gray-200 hover:border-gray-300 text-gray-600">
                            @if($user->status === 'active')
                ...
                <div>
                    <form action="{{ route('admin.account.destroy', $user) }}" method="POST" class="inline" onsubmit="return confirm('Bạn có chắc chắn muốn xóa người dùng này?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class=" p-3 flex items-center btn border-red-200 hover:border-red-300 text-red-600">
